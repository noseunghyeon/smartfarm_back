...
async def get_news_links(list_id: int = Query(19, alias="list")):
    """
    뉴스 관련 링크를 크롤링하여 반환합니다.
    각 뉴스 기사에 대해 이미지 URL과 내용도 함께 추가합니다.
    Query parameter 'list'를 통해 크롤링할 URL 번호를 지정합니다.
      - /news-links?list=19 : SaleNews (기본값)
      - /news-links?list=20 : Test4 등 다른 페이지에서 사용
    """
    try:
        # 전달받은 list_id를 통해 크롤링 대상 URL 생성
        url = f"https://www.nongmin.com/list/{list_id}"
        news_links = crawler.scrape_news_links(url)
