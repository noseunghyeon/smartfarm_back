...
try:
    ...
    # 가격에서 쉼표 제거하고 숫자로 변환
    df['onion'] = df['onion'].astype(str).str.replace(',', '').astype(float)
    ...
    # 가격 관련 특성 추가
    df['price_ma3'] = df['onion'].rolling(window=3, min_periods=1).mean()  # 3일 이동평균
    df['price_ma7'] = df['onion'].rolling(window=7, min_periods=1).mean()  # 7일 이동평균
    df['price_ma30'] = df['onion'].rolling(window=30, min_periods=1).mean()  # 30일 이동평균
    df['price_std7'] = df['onion'].rolling(window=7, min_periods=1).std()  # 7일 표준편차
    
    # 가격 변화율 추가
    df['price_change'] = df['onion'].pct_change()
    ...
    def predict_prices(weather_data=None):
        try:
            ...
            latest_price = df_sorted['onion'].iloc[0]
            ...
            def prepare_prediction_data(target_date):
                new_data = pd.DataFrame({
                    ...
                    'price_std7': [df_sorted['onion'].head(7).std()],
                    'price_change': [0],
                    'price_change_ma7': [0]
                })
