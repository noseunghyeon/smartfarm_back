...
try:
    ...
    # 이동평균 특성 추가
    df['price_ma7'] = df['paprika'].rolling(window=7).mean()
    df['price_ma30'] = df['paprika'].rolling(window=30).mean()
    
    # 필요한 특성 선택
    features = ['avg temp', 'max temp', 'min temp', 'rainFall', 'month', 'day', 
               'season', 'temp_diff', 'price_ma7', 'price_ma30']
    X = df[features]
    y = df['paprika']
    ...
    def predict_price(temp_avg, temp_max, temp_min, rainfall, month=datetime.now().month):
        ...
        price_ma7 = df['paprika'].mean()
        price_ma30 = df['paprika'].mean()
